...
namespace InteractiveWater
{
    ...
    public class InteractiveWater : MonoBehaviour
    {
        ...
        private void GenerateWaterMeshes()
        {
            ...
            {
                bc = transform.gameObject.AddComponent<BoxCollider>();
            }
            bc.isTrigger = true;
            // el top mesh va de (0..X) y (0..Z), con Y=0 ⇒ centramos el box
            bc.center = new Vector3(_topSurfaceSize.x / 2f, 0f, _topSurfaceSize.y / 2f);
            // lámina muy delgada en Y
            bc.size = new Vector3(_topSurfaceSize.x, 0.02f, _topSurfaceSize.y);
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            bool isInitialRippleUp = Random.value > 0.5f;

            var rb = other.attachedRigidbody; 
            if (rb != null)
                isInitialRippleUp = rb.linearVelocity.y > 0f;
